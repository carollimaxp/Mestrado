...
do 
	...
	do 
		...
		if [ $? -ne 0 ]; then
			...
			echo $linha >> /home/facom/Resultados/Transaction/INTER/Projeto/$filtro1.txt
			#linha_aux=$linha
			#$(echo "$aux" | tr '\b, \b' ',')
			#linha1=$(echo $linha | sed 's/[ ]/\n/g')
			#echo $linha1
			#echo $linha >> '/home/facom/Resultados/Transaction/INTER/Diff/teste.txt'
		fi

	done<"$pop/projetosInline.txt" #nome das Bibliotecas
	i=$(($i-$i))

	sed 's/ /\n/g' /home/facom/Resultados/Transaction/INTER/Projeto/$filtro1.txt >> /home/facom/Resultados/Transaction/INTER/Diff/$filtro1.txt

done<"$pop/INTER.csv" #nome das Bibliotecas