	...
	while read -r linha || [[ -n "$linha" ]]
		do	
			...
			if [ $? -ne 0 ]; then
				...
				echo "$linhaL" > $dir/FirstImport/TOP/aux.txt
				ultimaL=$(tail -1 "$dir/FirstImport/TOP/aux.txt") #>> $dir/FirstImport/$filtro1.txt
				...
				echo "$1, $primeiro, $resto" >> $dir/FirstImport/TOP/$filtro1.txt
		...
		rm $dir/FirstImport/TOP/aux.txt

done<"/home/facom/Resultados/TOP.csv" #nome das Bibliotecas  